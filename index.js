...
function createSymbolProperty(property) {
  ...
  // Додамо властивість до об'єкту, ключем якої буде наш символ,
  // а значенням буде аргумент property
  // Повертаємо об'єкт
}
...
function createAndCompareTwoSymbols(value) {
  ...
  // та повертаємо результат порівняння двох символів
  let a = Symbol(value);
  let b = Symbol(value);
  return a === b;
}
...
function createSymbolForKey(key) {
  ...
  // Symbol.for() шукає символ в глобальному реєстру символів. Якщо символ з таким ключем
  // вже існує, він повертає його, в іншому випадку створює новий символ з даним ключем
  // та додає його в реєстр
...
function getSymbolKey(symbol) {
  let a = Symbol.keyFor(symbol);
  return a;
  // Використовуємо Symbol.keyFor(sym) для отримання ключа, який був використаний 
  //для створення символу та повертаємо його
}
